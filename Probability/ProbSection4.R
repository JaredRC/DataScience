...
x
x/180000 # $180,000 loan

# Code: Calculating interest rate for 1% probability of losing money
l <- loss_per_foreclosure
z <- qnorm(0.01) # Normal dist
x <- -l*( n*p - z*sqrt(n*p*(1-p)))/ ( n*(1-p) + z*sqrt(n*p*(1-p)))
x/180000    # interest rate, $180,000 loan
...
# This Monte Carlo simulation estimates the expected profit given an unknown probability of default  0.03≤𝑝≤0.05 , modeling the situation where an event changes the probability of default for all borrowers simultaneously.
p <- 0.04
x <- 0.05*180000
profit <- replicate(B, {
